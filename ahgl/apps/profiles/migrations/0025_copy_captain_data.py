...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for team in orm.Team.objects.all():
            if team.captain:
                membership, created = orm.TeamMembership.objects.get_or_create(team=team, profile=team.captain)
                membership.captain = True
                if created: # captains who are not players
    ...
    def backwards(self, orm):
        ...
        for team in orm.Team.objects.all():
            try:
                team.captain = team.team_membership.filter(captain=True)[0]
            except IndexError:
                pass
            else:
                team.save()
