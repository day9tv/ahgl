...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for membership in orm.TeamMembership.objects.all():
            membership.questions_answers = membership.profile.questions_answers
            membership.char_name = membership.profile.char_name
            membership.char_code = membership.profile.char_code
            membership.game_profile = membership.profile.bnet_profile
            membership.race = membership.profile.race
            membership.save()
    ...
    def backwards(self, orm):
        ...
        for profile in orm.Profile.objects.all():
            try:
                membership = profile.team_membership.all()[0]
            except IndexError:
                pass
            else:
                profile.questions_answers = membership.questions_answers
                profile.char_name = membership.char_name
                profile.char_code = membership.char_code
                profile.bnet_profile = membership.game_profile
                profile.race = membership.race
                profile.save()
