...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for match in orm.Match.objects.order_by('creation_date'):
            for round in orm.TournamentRound.objects.filter(teams__pk=match.home_team.pk).filter(teams__pk=match.away_team.pk).order_by('-stage'):
                # we've already placed a match between those teams in this round, so go to next one
                if round.matches.filter(home_team=match.home_team, away_team=match.away_team).count() or round.matches.filter(home_team=match.away_team, away_team=match.home_team).count():
                    continue
                match.tournament_round = round
                match.save()
    ...
    models = {
        ...
        'tournaments.match': {
            ...
            'tournament_round': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'matches'", 'null': 'True', 'to': "orm['tournaments.TournamentRound']"}),
