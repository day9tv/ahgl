...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for round_membership in orm.TeamRoundMembership.objects.all():
            round_membership.tiebreaker = round_membership.team.game_wins.filter(match__published=True, match__tournament_round=round_membership.tournamentround).count() \
                                    - round_membership.team.game_losses.filter(match__published=True, match__tournament_round=round_membership.tournamentround).count()
            round_membership.wins = round_membership.team.match_wins.filter(published=True, tournament_round=round_membership.tournamentround).count()
            round_membership.losses = round_membership.team.match_losses.filter(published=True, tournament_round=round_membership.tournamentround).count()
            round_membership.save()
    ...
    models = {
        ...
        'tournaments.teamroundmembership': {
            'Meta': {'unique_together': "(('tournamentround', 'team'),)", 'object_name': 'TeamRoundMembership', 'db_table': "'tournaments_tournamentround_teams'"},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'losses': ('django.db.models.fields.IntegerField', [], {'default': '0'}),
            'team': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['profiles.Team']"}),
            'tiebreaker': ('django.db.models.fields.IntegerField', [], {'default': '0'}),
            'tournamentround': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['tournaments.TournamentRound']"}),
            'wins': ('django.db.models.fields.IntegerField', [], {'default': '0'})
        },
        ...
        'tournaments.tournamentround': {
            ...
            'teams': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'rounds'", 'symmetrical': 'False', 'through': "orm['tournaments.TeamRoundMembership']", 'to': "orm['profiles.Team']"}),
