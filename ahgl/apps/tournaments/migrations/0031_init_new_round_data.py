...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for round in orm.TournamentRound.objects.all():
            round.stage_order = round.stage
            round.stage_name = round.name
            try:
                round.order = int(round.name.split()[-1])
            except:
                round.order = 1
            round.save() 
    ...
    def backwards(self, orm):
        ...
        for round in orm.TournamentRound.objects.all():
            round.name = " ".join(round.stage_name, str(round.order))
            round.stage = round.stage_order
            round.save()
