...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for game in orm.Game.objects.exclude(Q(winner__in=["","N"]) | Q(home_player__isnull=True) | Q(away_player__isnull=True)):
            game.winner_tmp = game.home_player if game.winner=="H" else game.away_player
            game.save()
    ...
    def backwards(self, orm):
        ...
        for game in orm.Game.objects.all():
            if not game.winner_tmp:
                game.winner = "N"
                game.save()
            elif game.home_player:
                game.winner = "H" if game.winner_tmp==game.home_player else "A"
                game.save()
